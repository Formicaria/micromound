...
4. **What comes immediately after that?** M5 — the ESP32 as a subordinate deterministic controller
   ...
   (`v0.9.18`–`v0.9.26`). `firmware/esp32` binds that HAL to ESP-IDF and compiles to three images under
   ESP-IDF v5.3.2 — a 1.0 MB Wi-Fi mound, a 296 KB serial-link mound, a 259 KB port server that is the
   Pi's hands; flashing one and running it against real hardware is what remains.
