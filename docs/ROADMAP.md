...
4. **What comes immediately after that?** M5 — the ESP32 as a subordinate deterministic controller
   ...
   device loop and the board layer (enrollment, transport, drivers, the service loop over a
   seven-function HAL) in C, with a whole session accepted by the host's own verifier
   (`v0.9.18`–`v0.9.22`). `firmware/esp32` binds that HAL to ESP-IDF and is written but not yet
   compiled on a bench; the bench build is what remains.
