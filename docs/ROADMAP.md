...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   **independent watchdog thread** that de-energizes and stops the mound if the service loop hangs, so
   ...
   and a durable evidence store so the proof a mound captures survives a reboot (`v0.9.12`). Still
   remaining — and both need real hardware: the analog/ADC real port and a libgpiod backing. The
   daemon runs against generic primitives, now with a real GPIO line that holds a digital actuation,
   an independent watchdog guarding it, a controller it can enroll with, and evidence that outlives a
   restart, until the two hardware ports land.
