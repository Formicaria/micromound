...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   (`v0.9.6`), device enrollment over that transport (`v0.9.7`), and the first real driver *port* —
   a Linux GPIO output over sysfs behind the generic digital actuator (`v0.9.8`). Still remaining: the
   analog/ADC real port, a libgpiod backing, and a timed-hold actuator. The daemon runs against
   generic primitives, now with a real GPIO line available for digital actuation, until the rest land.
