...
| **M4** — The Linux/Pi host and first real drivers | **In progress (`v0.9.2`–`v0.9.4`)** | Landing as slices. **`v0.9.2`:** a **file-backed `IStateStore`** (atomic per-key writes, restart-survivable) so operational state survives on real disk, and the two ordering duties `v0.9.1` left as no-ops — persist a terminal report **before** clearing its checkpoint, and cold-start de-energize to safe state. **`v0.9.3`:** the **driver-resolution seam** — `ManifestDriverComposer` turns a manifest's hardware bindings into configured drivers, fail-closed as a whole, plus the first two **generic driver primitives** (a digital actuator, an analog sensor) that specialize by capability and settings, not by device-specific type. **`v0.9.4`:** a shared **`MoundComposition`** (the one place a mound is wired) and a **`MoundHost`** that brings a mound up from a manifest over the file store, runs missions, and recovers across restarts — the simulator and the host now compose the identical runtime. **Remaining:** the host daemon's **service loop** (a real network transport to the controller, the sync-beat loop, signal-driven graceful shutdown, the timing watchdog); the **real Linux driver ports** (GPIO/ADC over sysfs/libgpiod/I2C/SPI) behind those primitives; and the evidence store's disk backing on the same file substrate. `v0.10.0` is reserved for the boundary where the host runs on a real device — not these slices. (A relational store like SQLite is deliberately *not* used: the `IStateStore` contract is a narrow string-keyed document store, and its own design note calls for a directory of files rather than a database.) |
...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   state store (`v0.9.2`), the driver-resolution seam and generic driver primitives (`v0.9.3`), and a
   `MoundHost` that composes and runs a mound from a manifest over that store (`v0.9.4`). Still
   remaining: the host daemon's service loop (network transport, sync beat, signal handling,
   watchdog) and the real Linux driver *ports* behind the primitives. Nothing turns a physical output
   on until those land, because the host is what de-energizes it on stop.
