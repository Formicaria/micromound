...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   daemon with a safe service lifecycle (`v0.9.5`), and a real HTTP sync transport to the controller
   (`v0.9.6`). Still remaining: device enrollment over that transport, and the real Linux driver
   *ports* behind the primitives. The daemon runs against generic primitives until those land.
