...
4. **What comes immediately after that?** M5 — the ESP32 as a subordinate deterministic controller
   ...
   `firmware/micromound-c` — encoder, signer, reader, validators, the capability kernel and the
   device loop in C, with a whole session accepted by the host's own verifier (`v0.9.18`–`v0.9.21`).
   What remains is the board: the ESP-IDF project that gives it a transport, drivers, a clock and
   a key store.
