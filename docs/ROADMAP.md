...
| **M4** — The Linux/Pi host and first real drivers | **In progress (`v0.9.2`, `v0.9.3`)** | Landing as substrate slices. **`v0.9.2`:** a **file-backed `IStateStore`** (atomic per-key writes, restart-survivable) so operational state survives on real disk, and with the store durable the two ordering duties `v0.9.1` left as no-ops — persist a terminal report **before** clearing its checkpoint, and cold-start de-energize to safe state. **`v0.9.3`:** the **driver-resolution seam** — `ManifestDriverComposer` turns a manifest's hardware bindings into configured drivers, fail-closed as a whole, plus the first two **generic driver primitives** (a digital actuator, an analog sensor) that specialize by capability and settings, not by device-specific type, over a narrow in-memory IO seam. **Remaining:** the headless `Micromound.Host` daemon made runnable (composition builder shared with the simulator, service loop, watchdog, host-owned safe-state de-energizing on stop/quiesce/expiry/fault/shutdown); the **real Linux driver ports** (GPIO/ADC over sysfs/libgpiod/I2C/SPI) behind those primitives; and the evidence store's disk backing on the same file substrate. `v0.10.0` is reserved for the boundary where the host runs on a real device — not these substrate slices. (A relational store like SQLite is deliberately *not* used: the `IStateStore` contract is a narrow string-keyed document store, and its own design note calls for a directory of files rather than a database.) |
...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   state store landed at `v0.9.2` (operational state survives a restart on real disk, terminal report
   persisted before its checkpoint is cleared), and the driver-resolution seam plus the first generic
   driver primitives at `v0.9.3` (a manifest's hardware bindings become configured drivers,
   fail-closed). Still remaining: a runnable `Micromound.Host`, and the real Linux driver *ports*
   behind those primitives. Nothing turns a physical output on until those land, because the host is
   what de-energizes it on stop.
