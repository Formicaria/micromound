...
4. **What comes immediately after that?** M5 — the ESP32 as a subordinate deterministic controller
   ...
   golden fixtures. Not a second colony. Its protocol layer and its kernel already exist:
   `firmware/micromound-c` — encoder, signer, reader, validators and the capability kernel in C,
   proven against the fixtures on the host (`v0.9.18`–`v0.9.20`).
