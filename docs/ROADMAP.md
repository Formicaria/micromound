...
**M4 and M5 are open on hardware alone.** Everything in both is built and host-verified; each is
waiting on the same single thing, a run against real hardware — `docs/DEPLOY.md` walked to the end on
a Pi for M4, any one of the three ESP32 images flashed for M5. Neither is waiting on code, and
neither can be closed from this repository.

**That is why the P0 phase ran before the bench rather than after it,** which is not what this
document originally said. P0 is correctness and recovery debt: ten defects found by reading the
seams, every one of them green through the whole test suite, the simulator and the eighteen
acceptance criteria. They are exactly the defects a bench finds the expensive way — one at a time,
with a relay wired to the end of them. Getting them closed before hardware is attached is the better
order; the framing below was rewritten in `v0.9.40` to say so, rather than leaving a heading that
claimed the phases came afterwards.
...
| **P0–P7** — Beyond the bench | **P0 in progress and nearly closed (`v0.9.29`–`v0.9.39`): P0.1–P0.5, P0.7, P0.8, P0.10 done; P0.6 and P0.9 half done. P1–P7 planned** | The milestones above answered *what has to be true before something can move*. The phases answer *what has to be true before someone else can depend on this*: correctness and recovery debt, an extension model, hardware qualification, and optional device packages that ship independently. See [the phase plan](#beyond-the-bench-the-phase-plan). |
...
## Beyond the bench: the phase plan

Milestones M0–M5 answered *what has to be true before something can move*. That question is
answered in software. The next one is different — **what has to be true before someone other than
its author can depend on this** — and it does not decompose into the same numbered milestones,
because the work is now a mix of correctness debt, an extension model, hardware qualification and
optional packages that must be able to ship independently.
...
**This section was titled "After the bench" until `v0.9.40`, and that was wrong twice over.** It was
wrong as a description — P0 ran from `v0.9.29` while the bench was still ahead — and it was wrong as
a plan. P0 is the correctness and recovery debt, and every row in it is a defect that the whole test
suite, the simulator and the eighteen acceptance criteria were green through. Those are precisely
the ones a bench would surface the expensive way: intermittently, with hardware attached, one at a
time. A mound whose stop survives a reboot, whose lease expires unattended, whose cooldown a clock
step cannot refund, and which refuses work it could not account for is a far better thing to put a
relay on the end of. So P0 belongs before the bench, and the heading now says the thing that
actually governs: these phases are ordered against each other, not against the hardware run.

**What the hardware run still gates** is `v0.10.0` and the closure of M4 and M5 — nothing in P0, and
nothing in P1. The milestone record above is history and stays as written.
...
- ~~**A wrong reading confirms an action** (P0.1).~~ **Closed at `v0.9.30`.** `WitnessAnt.Confirm`
  established only that a confirming observation existed, postdated the action and was fresh — never
  that its *value* agreed with the state the action was supposed to produce, so every layer above was
  reporting "something looked" rather than "what it saw agrees", and the eighteen acceptance criteria
  passed either way. A mission step now carries a postcondition (`expect`) and the Witness compares
  the reading against it; a disagreement demotes the record to `unverified` naming both sides.
- ~~**A restart resets the duty cycle** (P0.5).~~ **Closed at `v0.9.33` and `v0.9.38`.**
  `ActuationHistory` was in-memory only, so `min_off_s` and `max_rate_per_h` began empty on every
  boot and a reboot loop was a way to actuate as often as you liked. It persists under its own cache
  key now; and since `v0.9.38` each instant carries a monotonic stamp beside it, so a wall clock
  stepped forward cannot hand a budget back either. What remains is named in P0.5: across a restart
  the counter reset with the process, and that gap needs the controller.
- ~~**A hold is measured against a pre-sync wall clock** (P0.2).~~ **Closed at `v0.9.31`.** The tick
  took one `UtcNow` at the top and ran the blocking sync before `ServiceActuations(now)`, so a slow
  round trip was time the held line never saw. Holds are now serviced before the sync as well as
  after, the sync's cost is measured monotonically and added to the tick's clock, and a hold fires on
  whichever of the wall and monotonic deadlines comes first.
