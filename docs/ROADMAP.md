...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   **independent watchdog thread** that de-energizes and stops the mound if the service loop hangs, so
   a held line cannot stay hot behind a stuck loop (`v0.9.10`). Still remaining: the analog/ADC real
   port, a libgpiod backing, and the evidence store's disk backing. The daemon runs against generic
   primitives, now with a real GPIO line that holds a digital actuation and an independent watchdog
   guarding it, until the rest land.
