...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   **independent watchdog thread** that de-energizes and stops the mound if the service loop hangs, so
   ...
   Still remaining: the analog/ADC real port, a libgpiod backing, and the evidence store's disk
   backing. The daemon runs against generic primitives, now with a real GPIO line that holds a digital
   actuation, an independent watchdog guarding it, and a controller it can enroll with, until the rest
   land.
