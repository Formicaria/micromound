...
4. **What comes immediately after that?** M5 — the ESP32 as a subordinate deterministic controller
   ...
   (`v0.9.18`–`v0.9.25`). `firmware/esp32` binds that HAL to ESP-IDF and compiles to a 1.0 MB Wi-Fi
   image or a 296 KB serial-link image under ESP-IDF v5.3.2; flashing one and watching it enroll and
   beat is what remains.
