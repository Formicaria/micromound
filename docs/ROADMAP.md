...
| **P0.5** ⚠️ *mostly done in `v0.9.33`* | **A restart reset the duty cycle.** `ActuationHistory` is two in-memory dictionaries. Nothing saves or restores it, so `min_off_s` and `max_rate_per_h` — real safety limits — start empty on every boot. A 300 s cooldown is enforced before a restart and gone after one | **Done (`v0.9.33`):** `ActuationHistory` persists under its own cache key — its own, not folded into the authority snapshot, because it changes on a different cadence and outlives any particular charter by design — written after every path that can actuate and restored before anything may ask the hardware for more. **Remaining, and harder than it looks: a stepped clock.** A *forward* jump is the dangerous direction for the rate window (it ages starts out early and refreshes a spent budget); a backward one is already conservative, and `min_off_s` is conservative in both. Fixing it properly means aging the window on monotonic elapsed time as `v0.9.31` did for holds — and across a restart the gap is genuinely unknowable, so it needs an explicit policy decision rather than a mechanism. An earlier draft of this slice shipped a `max(now, latest_known)` reference for it; it was removed before release because it guarded the *backward* direction, which was never the risk, and its tests passed without it |
