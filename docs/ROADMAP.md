...
| **M4** — The Linux/Pi host and first real drivers | **In progress (`v0.9.2`–`v0.9.5`)** | Landing as slices. **`v0.9.2`:** a **file-backed `IStateStore`** (atomic per-key writes, restart-survivable) so operational state survives on real disk, plus persist-report-before-clear and cold-start de-energize. **`v0.9.3`:** the **driver-resolution seam** — `ManifestDriverComposer` turns a manifest's hardware into configured drivers, fail-closed, plus the first **generic driver primitives** (a digital actuator, an analog sensor). **`v0.9.4`:** a shared **`MoundComposition`** and a **`MoundHost`** that brings a mound up from a manifest over the file store, runs missions, and recovers across restarts. **`v0.9.5`:** a **runnable daemon** — a `MoundService` lifecycle (heartbeat, sync beat, watchdog, graceful safe shutdown) and a real `micromound` entry point; a safety trip escalates to a stop that survives a reboot. **Remaining:** a **real network transport** to the controller (the daemon runs offline until then); the **real Linux driver ports** (GPIO/ADC over sysfs/libgpiod/I2C/SPI) behind the primitives; a dedicated watchdog thread for a hung loop; and the evidence store's disk backing. `v0.10.0` is reserved for the boundary where the host runs on a real device against real hardware — not these slices. (A relational store like SQLite is deliberately *not* used: the `IStateStore` contract is a narrow string-keyed document store, and its own design note calls for a directory of files rather than a database.) |
...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   `MoundHost` that composes and runs a mound from a manifest over that store (`v0.9.4`), and a
   runnable daemon with a safe service lifecycle (`v0.9.5`). Still remaining: a real network transport
   to the controller, and the real Linux driver *ports* behind the primitives. The daemon runs offline
   against generic primitives until those land.
