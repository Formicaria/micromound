...
3. **What must exist before real hardware can move?** M4, **in progress**: the durable file-backed
   ...
   GPIO output over sysfs behind the generic digital actuator (`v0.9.8`) — and a digital actuator that
   *holds* its line for a real `on_s`, released on the loop's cadence and by the safe state (`v0.9.9`).
   Still remaining: the analog/ADC real port, a libgpiod backing, the evidence store's disk backing,
   and the dedicated watchdog thread that de-energizes a held line if the loop hangs — a safety
   prerequisite once actuations are held. The daemon runs against generic primitives, now with a real
   GPIO line that actually holds a digital actuation, until the rest land.
