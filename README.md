...
else; phase P0 nearly closed (`v0.9.29`–`v0.9.39`).** Protocol contracts, Ed25519 signing, frozen wire bytes, the
...
the relay and probe as kernel executors, and the service loop, all over a nine-function hardware
...
unbounded size. It was frequency: written on every sync beat whether or not a single id had changed,
which at a 15 s cadence is 889 MB a day of identical bytes onto an SD card, where the writes are what
wears out. Splitting it into 2,048 small documents, the way the queue was fixed, would have added
inodes and fsyncs to a problem it does not have. Most beats change nothing, so most beats now write
nothing.

**New in `v0.9.42`:** the safe-state bound `v0.9.39` added was wrong on a small machine, and CI
caught it on the very test written to pin it. Each bounded driver call ran on the thread pool, so a
blocked driver occupied a pool thread and the next driver's call waited for the pool to inject
another — about a second, longer than the bound. The second driver timed out too and its line stayed
live. The guarantee inverted itself exactly where it matters most: the smaller the machine, the more
completely one stuck driver took the others with it, and a Pi is a small machine. Each driver now has
a thread of its own, so it can only ever wedge itself.
...
the heading was wrong as a description, and wrong as a plan: P0 is the correctness and recovery debt,
and every row in it is a defect the whole test suite, the simulator and the eighteen acceptance
criteria were green through. Those are the ones a bench surfaces the expensive way, with hardware
attached. M4 and M5 are marked for what they are — built and host-verified, open on a hardware run
and nothing else — and three "known gaps" that closed releases ago are struck through. Documentation
...
wall clock — which moves for reasons other than time passing. A Pi or an ESP32 with no
battery-backed RTC boots believing it is 1970 and steps forward by decades on its first sync; at that
instant every cooldown read as elapsed and every rate window as empty. Each recorded instant now
carries a monotonic stamp beside it, and an entry's age is the smaller of what the two clocks claim.
The daemon and the board both do this; a bench with no real clock behind it keeps the wall-clock-only
behaviour on purpose, so not one frozen byte moved. Across a restart the counter reset with the
process, and that gap is named rather than papered over — only the controller could close it.
