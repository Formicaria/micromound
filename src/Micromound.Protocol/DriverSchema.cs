...
public static class DriverSchemaCatalog
{
    ...
    public static readonly DriverTypeSchema DigitalActuator = new()
    {
        ...
        [
            ...
            new() { Name = "link", Label = "Board link", Kind = SettingKinds.Text, Default = "", HardwareOnly = true, Advanced = true,
                    Help = "Set to a serial device (/dev/ttyUSB0) to put this line on a board that runs the port server over the link (PROTOCOL.md §12) instead of a local GPIO; 'pin' is then the board's pin and 'chip' is ignored. The board's compiled polarity must match active_high." },
    ...
    public static readonly DriverTypeSchema AnalogSensor = new()
    {
        ...
        [
            ...
            new() { Name = "link", Label = "Board link", Kind = SettingKinds.Text, Default = "", HardwareOnly = true, Advanced = true,
                    Help = "Set to a serial device (/dev/ttyUSB0) to read this channel from a board that runs the port server over the link (PROTOCOL.md §12) instead of an ADS1115; 'channel' is then the board's ADC channel and bus/address/gain are ignored." },
