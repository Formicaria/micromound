...
| `enroll-exchange.txt` | the PROTOCOL.md §3 enrollment exchange as `HttpEnrollmentClient` performs it: the exact request body for a fixed device, and the verdict + detail line for fifteen scripted controller responses. **`v0.9.30`:** the request body gains `features` — the mission semantics the build implements; the scripted device is reduced-profile, so it sends `[]`, which is why the C# test builds its client with `features: []` exactly as it already does with `driverSchemas: []` | `mm_enroll_request_body` must equal the `request:` line; `mm_enroll_read_response` must reach every `verdict:` in the `detail:` words (`test_board.c`) |
